...
router.get('/names', async (ctx, next) => {
  ...
  if (code) {
    try {
      let d = await controller.getByCode(code, user)
      if (d) {
        ctx.body = d
      } else {
        ctx.status = 404
    ...
    try {
      let ret = await controller.fetch(query, user)
      ctx.body = ret ? {
...
router.post('/import', body({ multipart: true }), async (ctx, next) => {
  ...
  let p = new Promise((resolve, reject) => {
    ...
    csv.fromStream(reader, {
      ...
      renameHeaders: true,
      headers: ['name',, 'type', 'category', 'region', 'address', 'zipcode', 'site', 'level', 'fund', 'staff', 'description', 'note', 'taxNum', 'bank', 'account', 'title', ]
    }).on('data', d => {
      let type = d.type
      d.type = TYPES[type] || type
      d.createdBy = userId
      d.billing = {
        taxNum: d.taxNum,
        bank: d.bank,
